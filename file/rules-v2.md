...
- 专有名词指特定且独一无二的个体、地点、组织、事件或事物。
    - 人名、神话人物怪物名、地名可以先在 [字统网-译名查询](https://zi.tools/?secondary=transliteration) 查找，有规范译名优先按规范译名翻译。
        - 未找到规范译名但有通用译名或常用译名，可按通用译名或常用译名翻译。
    ...
    - 如果没有找到译名，请自行拟定翻译，可音译也可根据特征命名。如果实在无法想出翻译，则暂时保留不译。
