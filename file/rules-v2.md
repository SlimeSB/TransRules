...
模组中出现的**文化专有项、架空概念、自造词、海外网络热梗**或**隐喻、暗示、特殊风格**等内容，常使译者常感到头疼，因为这些元素在中文中难以找到贴切的对应表达。  
然而这确实是一个庞大的问题，大到可以写好几本书详细阐述，故在此只能介绍常用的翻译技巧或要点。  
...
- **归化**：以中文中意思相同或相近的表达方式代替
    ...
    - **禁止在翻译中套用不适宜的烂梗**，尤其是带有较大负面影响的！
- **解释、括注**：如果很精炼简洁，也可以解释或括注。
    - `Ya Think It's a Bird?` -> `你觉得这是只鸟吗？`  
      `Obtain a Kiwi` -> `获得一个猕猴桃（几维果）`  
