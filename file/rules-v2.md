...
- 因精力有限、能力不足或其他原因，不得不提交半成品翻译时，应删去未翻译的内容。
    - 需要保留原文的内容不应删去。
    - 向模组官库直接提交翻译也应如此。
- 对旧版翻译，建议**仅在有误时修改**。不要频繁进行细微调整，说的过去即可。确实有错误的应当根据情况及时修正，以免成为历史遗留问题。
...
- 模组要遵循原版的术语，附属模组、联动模组要遵循主模组的术语。
    - 对于原版中出现的术语请对照 Minecraft Wiki 上的 [译名标准化](https://zh.minecraft.wiki/w/Minecraft_Wiki:译名标准化) 页面翻译。
        - 对于 1.19.2 及以下版本的模组翻译，需注意 [译名标准化/历史](https://zh.minecraft.wiki/w/Minecraft_Wiki:译名标准化/历史) 的译名更改。译名更改的影响范围为此版本和前两个大版本，例如 1.19 的译名更改也会同步至 1.18.X 和 1.17.X，但不会同步至 1.16.X。详情请参 [Crowdin#新版本](https://zh.minecraft.wiki/w/Crowdin?oldid=686835#新版本)（旧）
        - 1.19.3 及以上版本均可收到最新的语言文件变动，不必刻意区分，按最新版译名翻译即可。
        - 英语原文更改时，会生成一个新的键，而旧键会被保留以供旧版本使用。涉及原文修改的译名，请以对应版本游戏内实际译名为准。
        - **如果你被绕晕了，请打开游戏，以当前相应版本游戏内实际表现为准。**
...
对文化专有项的翻译最为困难，也最能体现译者功底。  
简单来说，文化专有项就是中文中没有的项，可以是词，也可以是句子或暗示等。这里也包括了那些模组中的特有概念或自造词，因为它们的处理方法是相似的。  
游戏中文化专有项翻译的难点，一是空间限制，由于游戏UI的空间有限，无法使用“译者按”、“脚注”或“解释”（如果够短可以）这样的虽然好用但占用空间较大的处理方式。二是强调一致性，多处出现的文化专有项不能舍去，且只能用一个词满足各个不同的语境。三是极其灵活，模组并不是严谨的出版物或文章，内容往往仅凭作者喜好，因此会遇到大量新词、梗、自造词，这些内容没有先前翻译可参考，尤其挑战译者能力。  
...
- 音译：
    - 全音译：
    - Slime 史莱姆 Creeper 苦力怕
    - 半音译：与其他处理方法合并使用，比如音译+类别词、音译+词根、音译+用途或特征等。
    - Jeep 吉普车 beer 啤酒
- 归化：以中文中的文化专有项代替
...
兵无常势，水无常形。在翻译时也不要仅局限于上述方法，语言是动态的、开放的。
...
| [[石田]] |       `\u9fec`       |                                                               |
| [[奥气]] |       `\u9feb`       |                                                               |
|    ……    | `\u22ef\u22ef`（⋯⋯） | `Shift+6` 输入的偏下的省略号 `……` 替换为垂直居中的省略号 `⋯⋯` |
|    ——    |    `\u2e3a`（⸺）     |    `Shift+_` 输入的两节破折号 `——` 替换为整体的破折号 `⸺`     |
