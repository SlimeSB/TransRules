...
- 标点符号应与原版 Minecraft 统一，并使用规范的中文全角标点。
...
- 现实中存在的动植物名称建议按以下规则翻译：
    - 以双名法或拉丁属名命名的，有正式译名采正式译名，没有的按拉丁词根直译并对人名地名音译。
    - 以俗名命名的，优先选择该物种国内常用译名，否则直译。若风格允许直接套用学名也可。
    - 同物异名的按相应学科处理办法统一名称，学界有争议的译者择一译名统一即可，不必完全严谨。
    - 因历史原因被错译、混用、错用的名称，若已长期通行且无歧义，应当继续沿用。（Acacia 金合欢）
    - 为游戏内搜索方便，建议使用模组辞典中频率高的译名。
...
- 文艺作品名如画名、曲名，一般保留原文不翻译。曲名有正式译名的，可以按正式译名翻译。
- 作为术语的专有名词还需遵守术语翻译规则。
...
语言从来不是静态的符号系统，而是一场永不停息的**动态的开放的**游戏。在这场游戏中，语言的意义并非由个体独断，而是在无数次的交流、误读、修正与共识中逐渐成形。澄清这一点后，我们发现评判翻译不应依赖抽象的理论标准，也不存在这样静态的评价标准，而应回归​**​实践中的多人共识​**。  
本指南是实践经验的结晶，且每隔一段时间修订一次，能够在一定程度上反映公众共识。  
当发生争议时，首先需要明白：没有一种翻译能取悦所有人。一个人的喜恶声音再大也不能成为判断依据。请先思考：
- 该译名是否违背了翻译指南？
- 是否会造成理解障碍？
- 是否保持了原作的风貌？
- 是否经得起时间的考验？
  
都行就不要急着批评，只是与预期不符。但是“这里必须在限定时间内确定译名”，追求完美或者说钻牛角尖是不可取的。
   -->
